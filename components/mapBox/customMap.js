...
const CustomMap = (value) => {
  ...
  // longitude: 74.180358
  useEffect(() => {
    value.value && value.value2 ? setShowIcon(true) : setShowIcon(false);
  }, [value]);
  ...
  return (
    <div className="map-box">
      ...
      >
        {/* start location */}
        {showIcon ? (
          <Marker coordinates={[value.value.longitude, value.value.latitude]} id={"ss"} anchor="bottom">
            <img src="/map-start.svg" alt="search start" className="icons" />
          </Marker>
        ) : null}

        {showIcon ? (
          <Marker coordinates={[value.value2.longitude, value.value2.latitude]} id={"22"} anchor="bottom">
